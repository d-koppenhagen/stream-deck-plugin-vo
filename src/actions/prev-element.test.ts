...
import { PrevElementAction } from "./prev-element";
import type { CommandService, CommandResult } from "../services/command-service";
...
function createMockKeyDownEvent() {
  ...
  } as unknown as Parameters<PrevElementAction["onKeyDown"]>[0];
}

describe("PrevElementAction", () => {
  let actionInstance: PrevElementAction;
  let mockCommandService: CommandService;
  let mockStateService: VoiceOverStateService;

  beforeEach(() => {
    actionInstance = new PrevElementAction();
  ...
  describe("onKeyDown", () => {
    it("sends VO+Left Arrow when VoiceOver is running", async () => {
