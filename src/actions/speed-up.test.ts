...
import { SpeedUpAction } from "./speed-up";
...
function createMockKeyDownEvent() {
  ...
  } as unknown as Parameters<SpeedUpAction["onKeyDown"]>[0];
}

describe("SpeedUpAction", () => {
  let actionInstance: SpeedUpAction;
  let mockStateService: VoiceOverStateService;

  beforeEach(() => {
    actionInstance = new SpeedUpAction();
  ...
  describe("onKeyDown", () => {
    it("increases speech rate when VoiceOver is running", async () => {
      vi.mocked(mockStateService.isRunning).mockResolvedValue(true);
      vi.mocked(mockStateService.increaseSpeechRate).mockResolvedValue(60);
      const ev = createMockKeyDownEvent();

      await actionInstance.onKeyDown(ev);

      expect(mockStateService.isRunning).toHaveBeenCalledOnce();
      expect(mockStateService.increaseSpeechRate).toHaveBeenCalledOnce();
    });
    ...
    it("does nothing when VoiceOver is not running", async () => {
      ...
      expect(mockStateService.isRunning).toHaveBeenCalledOnce();
      expect(mockStateService.increaseSpeechRate).not.toHaveBeenCalled();
    });

    it("does not throw when increaseSpeechRate fails", async () => {
      vi.mocked(mockStateService.isRunning).mockResolvedValue(true);
      vi.mocked(mockStateService.increaseSpeechRate).mockRejectedValue(
