...
import { RepeatLastPhraseAction } from "./repeat-last-phrase.js";
...
function createMockKeyDownEvent() {
  ...
  } as unknown as Parameters<RepeatLastPhraseAction["onKeyDown"]>[0];
}

describe("RepeatLastPhraseAction", () => {
  let actionInstance: RepeatLastPhraseAction;
  let mockCommandService: CommandService;
  let mockStateService: VoiceOverStateService;

  beforeEach(() => {
    actionInstance = new RepeatLastPhraseAction();
  ...
  describe("onKeyDown", () => {
    it("sends VO+Z key press when VoiceOver is running", async () => {
