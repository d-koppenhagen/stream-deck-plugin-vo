 ...
 * by sending VO+Z (Ctrl+Option+Z) via keystroke to ensure correct
 * character mapping regardless of keyboard layout.
 */
  ...
  override async onKeyDown(ev: KeyDownEvent): Promise<void> {
    try {
      ...
      const result = await this.commandService.executeAppleScript(
        'tell application "System Events" to keystroke "z" using {control down, option down}',
      );
      if (!result.success) {
