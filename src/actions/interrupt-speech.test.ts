...
import { InterruptSpeechAction } from "./interrupt-speech.js";
...
function createMockKeyDownEvent() {
  ...
  } as unknown as Parameters<InterruptSpeechAction["onKeyDown"]>[0];
}

describe("InterruptSpeechAction", () => {
  let actionInstance: InterruptSpeechAction;
  let mockCommandService: CommandService;
  let mockStateService: VoiceOverStateService;

  beforeEach(() => {
    actionInstance = new InterruptSpeechAction();
  ...
  describe("onKeyDown", () => {
    it("sends Ctrl key press when VoiceOver is running", async () => {
