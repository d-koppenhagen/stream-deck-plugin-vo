...
import { StartReadingAction } from "./start-reading.js";
...
function createMockKeyDownEvent() {
  ...
  } as unknown as Parameters<StartReadingAction["onKeyDown"]>[0];
}

describe("StartReadingAction", () => {
  let actionInstance: StartReadingAction;
  let mockCommandService: CommandService;
  let mockStateService: VoiceOverStateService;

  beforeEach(() => {
    actionInstance = new StartReadingAction();
  ...
  describe("onKeyDown", () => {
    it("sends VO+A key press when VoiceOver is running", async () => {
